...
// maroc
//	arm11_freq[0] = (apps_pll_freq*2)/16; // 156
	arm11_freq[0] = (apps_pll_freq*2)/12; // 208
//	arm11_freq[2] = (apps_pll_freq*3)/13; // 288
	arm11_freq[1] = (apps_pll_freq*2)/8; // 312
	arm11_freq[2] = (apps_pll_freq*2)/6; // 416
	arm11_freq[3] = (apps_pll_freq*3)/8; // 468
	arm11_freq[4] = (apps_pll_freq*3)/6; // 624
	arm11_freq[5] = (apps_pll_freq*2)/3; // 832
//	arm11_freq[8] = (apps_pll_freq*3)/4; // 936
//	arm11_freq[9] = (apps_pll_freq); // 1248

/* mine
	arm11_freq[0] = FREQ_MHZ(104);
	arm11_freq[1] = FREQ_MHZ(312);
	arm11_freq[2] = (apps_pll_freq*2)/3;
//	arm11_freq[2] = (apps_pll_freq*4)/3; */
...
/*	if (val == arm11_freq[0])
	...
	else if (val == FREQ_MHZ(845))
	{
		mode = 0x0F;
	} 
	else if (val == FREQ_MHZ(858))
	{
		mode = 0x0F;
	} 
	else if (val == FREQ_MHZ(871))
	{
		mode = 0x0F;
	} 
	else if (val == arm11_freq[2])
	...
	else if (val == FREQ_MHZ(1339))
	{
		mode = 0x0F;
	}
	else if (val == FREQ_MHZ(1586))
	{
		mode = 0x0F;
	} else
	{
	...
	/*we support only two modes - 0x0C/0x0F <- by SAMSUNG JELEK! :p */
...
/* maroc freq
	arm11_freq[0] = (apps_pll_freq*2)/16; // 156 0x09
	arm11_freq[1] = (apps_pll_freq*2)/12; // 208 0x0A
	arm11_freq[2] = (apps_pll_freq*3)/13; // 288 0x0B
	arm11_freq[3] = (apps_pll_freq*2)/8; // 312 0x0C
	arm11_freq[4] = (apps_pll_freq*2)/6; // 416 0x0D
	arm11_freq[5] = (apps_pll_freq*3)/8; // 468 0x0E
	arm11_freq[6] = (apps_pll_freq*3)/6; // 624 0x0F
	arm11_freq[7] = (apps_pll_freq*2)/3; // 832 0x10
	arm11_freq[8] = (apps_pll_freq*3)/4; // 936 0x11
	arm11_freq[9] = (apps_pll_freq); // 1248 0x12
*/
...
//	arm11_freq[1] = FREQ_MHZ(468);
//	arm11_freq[2] = FREQ_MHZ(624);
//	arm11_freq[-] = FREQ_MHZ(780);
//	arm11_freq[3] = (apps_pll_freq*2)/3;
/*	arm11_freq[5] = FREQ_MHZ(845);
	arm11_freq[6] = FREQ_MHZ(858);
	arm11_freq[7] = FREQ_MHZ(871);
	arm11_freq[8] = (apps_pll_freq*4)/3;
	arm11_freq[9] = FREQ_MHZ(1339);
	arm11_freq[10] = FREQ_MHZ(1586);
*/
...
unsigned long bcm21553_ahb_get_rate(struct clk *clk)
{
	u32 mode = 0, ahb;
	u32 apps_pll_freq = 0;
	mode = readl(ADDR_CLKPWR_CLK_ARMAHB_MODE) & 0x10;
	if(mode < 0x09)
		ahb = clk_armahb_reg_to_ahb_freq_mapping[mode];
	else
	{
